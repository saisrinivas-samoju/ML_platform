...
class EncryptData:
    ...
    def get_encrypted_text(self,text):
        ...
        Takes 'text' input and returns 'hash' as output using passlib library
        """
        try:
    ...
    def verify_encrypted_text(self,text,encrypted_text):
        """
        This function will take 'text' and 'encrypted_text', check if both are matching using passlib library.
        """
        try:
    ...
    def generate_key(self,):
        """
        Generates a key using Cryptography library and saves it into a file
        """
        key = Fernet.generate_key()
        #with open("secret.key", "wb") as key_file:
            #key_file.write(key)

        # We are going to save it to our environment variable, so that only we can access it
        key=key.decode('utf-8')
        return key

    def load_key(self):
        """
        Loads the secret key saved in our environment variable
        """
    ...
    def encrypt_message(self,message,key=None):
        """
        Encrypts a message using Cryptography library
        """
    ...
    def decrypt_message(self,encrypted_message,key=None):
        """
        Decrypts an encrypted message using Cryptography library
        """
        if key is None:
            key=self.load_key()
        f = Fernet(key)
